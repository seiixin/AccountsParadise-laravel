...
function AddModal({ open, onClose, onCreated }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70">
      <div className="w-full max-w-5xl rounded glass-soft p-6 max-h-[80vh] overflow-y-auto">
        <div className="text-sm text-neutral-400">Merchant</div>
        <div className="text-xl font-semibold">Add New Item</div>
        <div className="mt-4 grid grid-cols-2 gap-3">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Example: Valorant Premium Skin Bundle" className="rounded border border-neutral-800 bg-neutral-950 px-3 py-2 col-span-2" />
          ...
          <input value={price} onChange={(e) => setPrice(e.target.value)} placeholder="₱0.00" className="rounded border border-neutral-800 bg-neutral-950 px-3 py-2" />
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Delivery method, requirements, timeframe..." className="col-span-2 rounded border border-neutral-800 bg-neutral-950 px-3 py-2" />
          <div className="col-span-2">
            <div className="text-sm text-neutral-400 mb-1">Cover Image</div>
            <input type="file" accept="image/*" onChange={(e) => setCoverImage(e.target.files?.[0] ?? null)} />
          </div>
          <div className="col-span-2">
            <div className="text-sm text-neutral-400 mb-1">Gallery Images (up to 10)</div>
            <input type="file" multiple accept="image/*" onChange={(e) => setImages(Array.from(e.target.files || []))} />
          </div>
        </div>
        <div className="mt-6 flex justify-end gap-3">
          <button onClick={onClose} className="rounded border border-neutral-700 px-4 py-2">Cancel</button>
          <button onClick={save} disabled={saving} className="rounded bg-white px-4 py-2 text-black">Save</button>
        </div>
      </div>
    </div>
  , document.body);
...
function EditModal({ open, onClose, listingId, onUpdated }) {
  ...
  async function deleteImage(imageId) {
    await axios.delete(`/merchant/listings/${listingId}/images/${imageId}`, { headers: { Accept: 'application/json' }, withCredentials: true });
    setGallery(gallery.filter(i => i.id !== imageId));
  }

  async function setCover(imageId) {
    const res = await axios.put(`/merchant/listings/${listingId}/cover/${imageId}`, {}, { headers: { Accept: 'application/json' }, withCredentials: true });
  ...
  async function deleteCover() {
    const res = await axios.delete(`/merchant/listings/${listingId}/cover`, { headers: { Accept: 'application/json' }, withCredentials: true });
    setListing(prev => ({ ...prev, cover_image_path: res.data.cover_image_path }));
  }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/70">
      <div className="w-full max-w-6xl rounded glass-soft p-6 max-h-[80vh] overflow-y-auto">
        <div className="text-sm text-neutral-400">Merchant</div>
        ...
        {(
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              ...
              {!!images.length && (
                <div className="mt-3">
                  <div className="text-sm text-neutral-400">Pending Uploads ({images.length})</div>
...
export default function Listings({ initial }) {
  ...
  return (
    <MerchantLayout title="Merchant · Listings" header={<h2 className="text-xl font-semibold">Merchant · Listings</h2>}>
      ...
      <div className="mb-4 flex gap-2">
        ...
        <button onClick={() => setAddOpen(true)} className="rounded glass-soft px-3 py-2">Add Listing</button>
        <a href="/store" className="rounded glass-soft px-3 py-2">Preview Store</a>
      </div>
      <div className="overflow-hidden rounded-lg border border-neutral-800 glass-soft">
            ...
            {data.map(l => (
              <tr key={l.id}>
                ...
                <td className="px-4 py-2">
                  <div className="landscape-box bg-neutral-800" style={{ maxWidth: 160 }}>
                    {l.cover_image_path ? <img src={`/storage/${l.cover_image_path}`} alt="" /> : null}
                  </div>
                </td>
                ...
                <td className="px-4 py-2">
                  <button onClick={() => { setSelectedId(l.id); setEditOpen(true); }} className="rounded border border-neutral-700 px-3 py-1">Edit</button>
                </td>
              </tr>
            ))}
            {!data.length && (
              <tr>
                <td className="px-4 py-6 text-center text-neutral-400" colSpan={8}>No listings</td>
              </tr>
            )}
          </tbody>
        </table>
